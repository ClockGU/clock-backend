...
aussieht sind hier `hier <https://github.com/ambv/black#the-black-code-style>`_  zu finden.
...
aus, bevor Git tatsächlich einen Commit durchführt. Um dies zu tun, ist das Pythonprogramm `pre-commit
<https://pre-commit.com/>`_ nötig. Dies kann simpel über `sudo pip install pre-commit` installiert werden. Anschließend
muss nur noch der vom Projekt in `.pre-commit-config.yaml` definierte Hook installiert werden. Dies geschieht durch
`pre-commit install`. Nun muss man sich keine Gedanken mehr über Black machen. Bei jedem Commit wird der Black Formatter über den Code geschickt.
...
Bis jetzt (Stand Dez. 2018) ist Celery nur in der lokalen Entwicklungsumgebung durch `docker-compose` zugänglich.
Der Grund hierfür ist, dass das von uns verfolgte Prinzip "Celeryumgebung abgekoppelt von Bussinesumgebung", also zwei Container, Web-Container und Celery-Container, noch (Stand Okt. 2018) nicht auf Dokku übertragbar ist.
