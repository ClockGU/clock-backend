        ...
        try:
            ...
            if isinstance(error, InvalidToken):
                raise serializers.ValidationError(
                    "Invalid key: Key was not generated by the System."
                )
            if isinstance(error, JSONDecodeError):
                raise serializers.ValidationError(
                    "Invalid key: Decrypted data is not JSON serializable."
                )
        try:
            ...
            raise serializers.ValidationError(
                "Invalid Data: Decrypted data does not contain necessary information."
            )
...
class ReportingEndpoint(generics.GenericAPIView):
    ...
    def get(self, request, month=None, year=None):
        user_query = (
            f"references={'&references='.join(request.user.supervised_references)}"
        )
